...
def get_league_in_session(session: Session, league_id: int, rosters: bool = True) -> League:
    if rosters:
        league = session.query(League).options(joinedload(League.teams).joinedload(Team.roster_spots).joinedload(Roster_Spot.player)).filter_by(id=league_id).first()
    ...
    if updated_teams is None or inflation is not None:
        ...
        for team in league.teams:
            if __list_contains_team(team, updated_teams):
                if not league.is_salary_cap():
                    inflation = None
                __project_team_results(
                    team, league, value_calc, league.s_format, fill_pt, inflation, stats=stats, accrued_pt=pt, keepers=keepers, use_keepers=use_keepers, custom_scoring=custom_scoring
                )
...
def __project_team_results(
    ...
    if fill_pt:
        ...
        if ScoringFormat.is_h2h(s_format):
            pt = roster_services.optimize_team_pt(team, league, keepers, value_calc, s_format, use_keepers=use_keepers, custom_scoring=custom_scoring)
        else:
            pt = roster_services.optimize_team_pt(team, league, keepers, value_calc, s_format, use_keepers=use_keepers, custom_scoring=custom_scoring)
    else:
        if ScoringFormat.is_h2h(s_format):
...
def get_league_inflation(league: League, inf_method: InflationMethod) -> float:
    ...
    if inf_method == InflationMethod.CONVENTIONAL:
        ...
        league.inflation = (salary_cap - roster_spots * league.num_teams - (league.total_salary - league.num_rostered)) / (
            salary_cap - roster_spots * league.num_teams - (league.total_value - league.num_rostered)
        ) - 1
    else:
