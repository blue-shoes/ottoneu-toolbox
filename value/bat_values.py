...
class BatValues:
    ...
    def __init__(
        ...
        if self.starting_pos:
            self.position_keys = [
                p.position for p in self.starting_pos.positions if p.position.offense
            ]
            self.start_count = dict(
                [(p.position, p.count) for p in self.starting_pos.positions]
            )
        else:
    ...
    def rank_position_players(self, df: DataFrame, rank_col: str = None) -> None:
        ...
        for pos in self.position_keys:
            ...
            if pos == P.POS_UTIL:
                ...
                df[col] = df.loc[
                    df['Position(s)'].apply(
                        lambda test_pos, _pos=pos: P.eligible(test_pos, _pos)
                    )
                ][rank_col].rank(ascending=False)
                df[col] = df[col].fillna(-999)
                self.max_rost_num[pos.value] = len(
                    df.loc[
                        df['Position(s)'].apply(
                            lambda test_pos, _pos=pos: P.eligible(test_pos, _pos)
                        )
    ...
    def calc_pos_games(self, row, pos: P) -> int:
        ...
        if not P.eligible(row['Position(s)'], pos):
            return 0
        if (self.__position_is_base(pos) or pos == P.POS_UTIL) and row[
            f'{pos.value}_FOM'
        ] < 0:
            return 0
        if pos == P.POS_MI:
        ...
        if pos == P.POS_INF:
            if (
                ...
                return 0
        if pos == P.POS_OF and all(
            x in [P.POS_LF, P.POS_CF, P.POS_RF] for x in self.position_keys
        ):
    ...
    def are_games_filled(
        self, df: DataFrame, multi_pos: P = None, multi_pos_idx: int = 0
    ) -> bool:
        ...
        for pos in self.position_keys:
            if self.__position_is_base(pos):
                excess_games = (
                    self.total_games[pos.value]
                    - num_teams * self.target_games * self.start_count[pos]
                )
                excess_games_dict[pos.value] = excess_games
                if (
                    excess_games < 0
                    and self.max_rost_num[pos.value]
                    > self.replacement_positions[pos.value]
                ):
            ...
            if not self.__position_is_base(pos):
                if pos == P.POS_UTIL:
                    ...
                    position_count = (
                        sum(
                            [
                                self.start_count.get(sub_pos[0], 0)
                                for sub_pos in pos.component_pos
                            ]
                        )
                        + self.start_count[pos.value]
                    )
                    total_game_sum = sum(
                        [
                            self.total_games.get(sub_pos[0], 0)
                            for sub_pos in pos.component_pos
                        ]
                    )
                    excess_games = (
                        total_game_sum - num_teams * self.target_games * position_count
                    )
                    excess_games_dict[pos.value] = excess_games
                    for sub_pos in pos.component_pos:
                        component_excess[sub_pos[0]] = excess_games_dict.pop(
                            sub_pos[0], 0
                        )
                    if excess_games < 0:
                        for sub_pos in pos.component_pos:
                            if P.position_is_base(
                                P._value2member_map_[sub_pos[0]], self.position_keys
                            ):
                                if self.max_rost_num.get(
                                    sub_pos[0], 0
                                ) > self.replacement_positions.get(sub_pos[0], 0):
        ...
        for pos, excess in excess_games_dict.items():
            if pos in self.replacement_levels and self.replacement_levels[
                pos
            ] == self.replacement_levels.get(P.POS_UTIL.value, None):
                continue
        ...
        if not non_util_filled and max_excess > self.target_games:
            ...
            if not P.position_is_base(pos, self.position_keys):
                ...
                for sub_pos in pos.component_pos:
                    if component_excess.get(sub_pos[0], -999) > max_sub_excess:
                        if sub_pos[
                            0
                        ] in self.replacement_levels and self.replacement_levels[
                            sub_pos[0]
                        ] == self.replacement_levels.get(P.POS_UTIL.value, None):
                            continue
            ...
            while index > 0:
                ...
                for elig in eligibilities:
                    if elig not in self.games_filled:
                        continue
                    if elig == pos_val:
                        continue
                ...
                if test_eligibilities:
                    ...
                    return self.are_games_filled(
                        df, multi_pos=adj_pos, multi_pos_idx=index
                    )
                index -= 1
        return filled_games
    ...
    def are_util_games_filled(self, num_teams: int = 12) -> bool:
        ...
        for pos in self.position_keys:
            ...
            if self.__position_is_base(pos) or pos == P.POS_UTIL:
                total_games += (
                    self.total_games[pos.value]
                    - self.target_games * num_teams * self.start_count[pos]
                )
            else:
        ...
        return (
            total_games >= 0
            and self.max_rost_num['Util'] > self.replacement_positions['Util']
        )
        ...
        if self.rep_level_scheme == RepLevelScheme.STATIC_REP_LEVEL:
            ...
            for pos in self.position_keys:
                if self.__position_is_base(pos) or pos == P.POS_UTIL:
                    if (
                        self.replacement_positions[pos.value]
                        > self.max_rost_num[pos.value]
                    ):
                        self.replacement_positions[pos.value] = self.max_rost_num[
                            pos.value
                        ]
            ...
            if self.rep_level_scheme == RepLevelScheme.FILL_GAMES:
                ...
                while not self.are_games_filled(df):
                    last_inc += 1
                    if (
                        last_inc % int(self.num_teams / 3) == 0
                        and self.progress_updater.progress < 70
                    ):
                    ...
                    if max_rep_lvl == -999:
                        ...
                        for pos in self.position_keys:
                            ...
                            if rep_lvl > max_rep_lvl:
                                max_rep_lvl = rep_lvl
                                max_pos = pos
                    self.replacement_positions[max_pos.value] = (
                        self.replacement_positions[max_pos.value] + 1
                    )
                    ...
                    if num_bats > self.target_bat:
                        ...
                        for pos in self.position_keys:
                            ...
                            if rep_lvl < min_rep_lvl:
                                min_rep_lvl = rep_lvl
                                min_pos = pos
                        self.replacement_positions[min_pos.value] = (
                            self.replacement_positions[min_pos.value] - 1
                        )
                        if not ScoringFormat.is_points_type(self.s_format):
                            self.calculate_roto_bases(df)
                        ...
                        for pos in self.position_keys:
                            ...
                            if rep_lvl > max_rep_lvl:
                                ...
                                if (
                                    pos == P.POS_1B
                                    and self.replacement_positions['1B']
                                    > 1.5 * self.replacement_positions['SS']
                                ):
                                    continue
                                if (
                                    pos == P.POS_OF
                                    and self.replacement_positions['OF']
                                    > 3 * self.replacement_positions['SS']
                                ):
                                    continue
                                max_rep_lvl = rep_lvl
                                max_pos = pos
                ...
                # shouldn't get here
                logging.error(
                    f'Inappropriate Replacement Level Scheme {self.rep_level_scheme}'
                )
    ...
    def set_num_rostered_from_rep_level(self, df: DataFrame, pos: P) -> None:
        ...
        while True:
            ...
            if (
                rate < self.replacement_levels[pos.value]
                and (index + 1) < self.max_rost_num[pos.value]
            ):
                break
            index += 1
    ...
    def get_fom_from_rep_level(self, df: DataFrame, pos: P) -> None:
        ...
        col = pos.value + '_FOM'
        df[col] = df.apply(
            self.calc_bat_fom, args=(self.replacement_levels[pos.value], pos), axis=1
        )
        if pos.value in ['SS', '2B']:
            rep_level = max(
                self.get_position_rep_level(df, P.POS_SS),
                self.get_position_rep_level(df, P.POS_2B),
            )
            df['MI_FOM'] = df.apply(
                self.calc_bat_fom, args=(rep_level, P.POS_MI), axis=1
            )
        if pos.value in ['1B', '3B']:
            rep_level = max(
                self.get_position_rep_level(df, P.POS_1B),
                self.get_position_rep_level(df, P.POS_3B),
            )
            df['CI_FOM'] = df.apply(
                self.calc_bat_fom, args=(rep_level, P.POS_CI), axis=1
            )
        if pos.value in ['1B', '2B', 'SS', '3B']:
            ...
            df['INF_FOM'] = df.apply(
                self.calc_bat_fom, args=(rep_level, P.POS_INF), axis=1
            )
        if pos.value in ['LF', 'CF', 'RF'] and not self.__position_is_base(P.POS_OF):
            rep_level = max(
                self.get_position_rep_level(df, P.POS_LF),
                self.get_position_rep_level(df, P.POS_RF),
                self.get_position_rep_level(df, P.POS_CF),
            )
            df['OF_FOM'] = df.apply(
                self.calc_bat_fom, args=(rep_level, P.POS_OF), axis=1
            )
    ...
    def get_position_rep_level(self, df: DataFrame, pos: P) -> float:
        ...
        if pos != P.POS_UTIL:
            ...
            return pos_df.iloc[self.replacement_positions[pos.value] - 1][
                self.rank_basis
            ]
        else:
    ...
    def calculate_roto_bases(self, proj: DataFrame, init=False) -> None:
        ...
        if RankingBasis.is_roto_fractional(
            RankingBasis.get_enum_by_display(self.rank_basis)
        ):
            ...
            if self.s_format == ScoringFormat.CUSTOM:
                ...
                for cat in self.scoring.stats:
                    ...
                    if cat.category.rate_denom is None:
                        proj[f'{cat.category.display}/G'] = proj.apply(
                            self.per_game_rate, axis=1, args=(cat.category,)
                        )
                        ...
                        self.stat_avg[cat.category] = dataframe_util.weighted_avg(
                            ...
                            f'{cat.category.rate_denom.display}/G',
                        )
                ...
                proj['HR/G'] = proj.apply(
                    self.per_game_rate, axis=1, args=(StatType.HR,)
                )
                if self.s_format == ScoringFormat.OLD_SCHOOL_5X5:
                    proj['RBI/G'] = proj.apply(
                        self.per_game_rate, axis=1, args=(StatType.RBI,)
                    )
                    proj['SB/G'] = proj.apply(
                        self.per_game_rate, axis=1, args=(StatType.SB,)
                    )
                    self.stat_avg[StatType.AVG] = dataframe_util.weighted_avg(
                        above_rep_lvl, 'AVG', 'AB/G'
                    )
                    proj['AVG_Delta'] = proj.apply(
                        self.calc_rate_delta, axis=1, args=(StatType.AVG,)
                    )
                    cat_to_col = {
                        StatType.R: 'R/G',
                        StatType.HR: 'HR/G',
                        StatType.RBI: 'RBI/G',
                        StatType.SB: 'SB/G',
                        StatType.AVG: 'AVG_Delta',
                    }
                else:
                    self.stat_avg[StatType.OBP] = dataframe_util.weighted_avg(
                        above_rep_lvl, 'OBP', 'PA/G'
                    )
                    proj['OBP_Delta'] = proj.apply(
                        self.calc_rate_delta, axis=1, args=(StatType.OBP,)
                    )
                    self.stat_avg[StatType.SLG] = dataframe_util.weighted_avg(
                        above_rep_lvl, 'SLG', 'AB/G'
                    )
                    proj['SLG_Delta'] = proj.apply(
                        self.calc_rate_delta, axis=1, args=(StatType.SLG,)
                    )
                    cat_to_col = {
                        StatType.R: 'R/G',
                        StatType.HR: 'HR/G',
                        StatType.OBP: 'OBP_Delta',
                        StatType.SLG: 'SLG_Delta',
                    }
        else:
            ...
            if self.s_format == ScoringFormat.CUSTOM:
                ...
                for cat in self.scoring.stats:
                    ...
                    if cat.category.rate_denom is not None:
                        self.stat_avg[cat.category] = dataframe_util.weighted_avg(
                            above_rep_lvl, cat.category.display, cat.category.display
                        )
                        proj[f'{cat.category.display}_Delta'] = proj.apply(
                            self.calc_rate_delta, axis=1, args=(cat.category,)
                        )
                ...
                if self.s_format == ScoringFormat.OLD_SCHOOL_5X5:
                    ...
                    self.stat_avg[StatType.OBP] = dataframe_util.weighted_avg(
                        above_rep_lvl, 'OBP', 'PA'
                    )
                    proj['OBP_Delta'] = proj.apply(
                        self.calc_rate_delta, axis=1, args=(StatType.OBP,)
                    )
                    self.stat_avg[StatType.SLG] = dataframe_util.weighted_avg(
                        above_rep_lvl, 'SLG', 'AB'
                    )
                    proj['SLG_Delta'] = proj.apply(
                        self.calc_rate_delta, axis=1, args=(StatType.SLG,)
                    )
    ...
    def calc_rate_delta(self, row, stat: StatType) -> float:
        ...
        if stat.rate_denom == StatType.PA:
            ...
            raise ValueError(
                f'Offensive stat {stat.name} did not have a usable rate denominator stat'
            )
        if RankingBasis.is_roto_fractional(
            RankingBasis.get_enum_by_display(self.rank_basis)
        ):
    ...
    def calc_z_score(self, row) -> float:
        ...
        if self.s_format == ScoringFormat.CUSTOM:
            for cat in self.scoring.stats:
                ...
                if cat.category.rate_denom is None:
                    ...
                    zScore += (
                        mult
                        * (
                            row[f'{cat.category.display}{col_add}']
            ...
            zScore += (
                (row[f'R{col_add}'] - self.stat_avg.get(StatType.R))
                / self.stat_std.get(StatType.R)
                * rat
            )
            zScore += (
                (row[f'HR{col_add}'] - self.stat_avg.get(StatType.HR))
                / self.stat_std.get(StatType.HR)
                * rat
            )
            if self.s_format == ScoringFormat.OLD_SCHOOL_5X5:
                zScore += (
                    (row[f'RBI{col_add}'] - self.stat_avg.get(StatType.RBI))
                    / self.stat_std.get(StatType.RBI)
                    * rat
                )
                zScore += (
                    (row[f'SB{col_add}'] - self.stat_avg.get(StatType.SB))
                    / self.stat_std.get(StatType.SB)
                    * rat
                )
    ...
    def calc_fom(self, pos_proj: DataFrame, min_pa: int) -> DataFrame:
        ...
        if (
            self.scoring is not None and self.scoring.points_format
        ) or ScoringFormat.is_points_type(self.s_format):
