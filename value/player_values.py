...
class PlayerValues:
    def __init__(self, value_calc: ValueCalculation, debug=False, rostered_hitters=244, rostered_pitchers=196, rp_limit=999):
    ...
    def calculate_values(self, progress:ProgressUpdater=None) -> None:
        ...
        pos_values = value.bat_values.BatValues(self.value_calc, intermediate_calc=self.intermediate_calculations, prog=progress)
        ...
        if self.intermediate_calculations:
            filepath = os.path.join(self.intermed_subdirpath, 'pos_value_detail.csv')
            pos_min_pa.to_csv(filepath, encoding='utf-8-sig')
            filepath = os.path.join(self.intermed_subdirpath, 'pitch_value_detail.csv')
