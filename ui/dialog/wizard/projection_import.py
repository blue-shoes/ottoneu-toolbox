                ...
                if self.proj_type.get() == ProjectionType.DAVENPORT.type_name:
                    ...
                    if not os.path.exists('conf/fangraphs.conf'):
                        ...
                        if dialog.status == CANCEL:
                            ...
                            return False
                # Download proj
                self.hitter_df, self.pitcher_df = projection_services.create_projection_from_download(
                    self.parent.projection, ProjectionType.get_enum_by_name(self.proj_type.get()), self.ros_var.get(), self.dc_var.get(), year=year, progress=pd
                )
            else:
                # Upload proj from files
                self.hitter_df, self.pitcher_df = projection_services.create_projection_from_upload(
                    self.parent.projection, self.hitter_proj_file.get(), self.pitcher_proj_file.get(), name='User Custom', year=year, progress=pd
                )
...
class DavenportHyperlinkDialog(tk.Toplevel):
    def __init__(self, parent):
        ...
        text_widget = Text(self, width=45, height=4, wrap=tk.WORD, font=font.nametofont('TkDefaultFont'))
