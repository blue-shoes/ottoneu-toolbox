...
def get_draft_by_league(lg_id: int) -> Draft:
    ...
    with Session() as session:
        draft = session.query(Draft).filter_by(league_id=lg_id, year=year).first()
        if draft is None:
            draft = Draft()
...
def create_target(draft_id: int, playerid: int, price: int) -> Draft_Target:
    ...
    with Session() as session:
        ...
        session.add(target)
        session.commit()
        target = session.query(Draft_Target).filter_by(id=target.id).options(joinedload(Draft_Target.player)).first()
    return target
