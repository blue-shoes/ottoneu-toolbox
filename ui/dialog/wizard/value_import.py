    ...
    def finish(self):
        try:
            ...
            if self.step0.value_type.get() == ValueTypeEnum.FG_AUCTION_CALC:
                ...
                progd.set_completion_percent(15)
                self.value = calculation_services.save_calculation_from_file(
                    self.value, self.step1.df, progd, rep_val=int(self.step1.rep_level_value_str.get()), new_pos_set=self.step2.use_file_pos_bv.get()
                )
...
class Step1(tk.Frame):
    ...
    def validate(self):
        ...
        try:
            ...
            self.parent.validate_msg = calculation_services.normalize_value_upload(
                df, ScoringFormat.get_format_by_full_name(self.game_type.get()), id_type=IdType._value2member_map_.get(self.id_type.get(), None)
            )
        except PermissionError:
            self.parent.validate_msg = 'Error loading values file. File permission denied.'
            return False
    ...
    def init_value_calc(self):
        ...
        calculation_services.init_outputs_from_upload(
            vc, self.df, ScoringFormat.get_format_by_full_name(self.game_type.get()), int(self.rep_level_value_str.get()), IdType._value2member_map_.get(self.id_type.get()), prog
        )
        prog.complete()
