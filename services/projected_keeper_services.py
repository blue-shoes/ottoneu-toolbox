...
def get_league_keepers(league: League):
    with Session() as session:
        keepers = session.query(Projected_Keeper).filter(Projected_Keeper.league_id == league.id).filter(Projected_Keeper.season == date_util.get_current_ottoneu_year()).all()
        if keepers is None:
            keepers = []
